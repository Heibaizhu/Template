...
class SMFAModel(BaseModel):
    ...
    def loss_fn(self, output):
        ...
        # outDepths =output['depth']

        # N =len(dehazed)
        #
        # predJ = dehazed[-1]
        # outDepth = outDepths[-1]
    ...
    def core_validation(self, dataloader, current_iter,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            for batch_index in range(len(img_names)):
                haze_imgs.append(tensor2img([visuals['haze'][batch_index]], rgb2bgr=False, min_max=(0, 1)))
                clear_imgs.append(tensor2img([visuals['clear'][batch_index]], rgb2bgr=False, min_max=(0, 1)))
                dehazed_imgs.append(tensor2img([visuals['dehazed'][batch_index]], rgb2bgr=False, min_max=(0, 1)))

            # del self.haze
            # del self.clear
            torch.cuda.empty_cache()
            # io.imsave("/mnt/data/zhajunwei/JDRNN2/temp.png", dehazed_imgs[0])
            # imwrite(dehazed_imgs[0], "/mnt/data/zhajunwei/JDRNN2/temp.png")
            ...
            if save_img:
                for haze_img, clear_img, dehazed_img, img_name in zip(haze_imgs, clear_imgs, dehazed_imgs, img_names):
                    ...
                    dehazed_img_path = base_path + '_img_dehazed.png'

                    # imwrite(haze_img, haze_img_path)
                    # imwrite(clear_img, clear_img_path)
                    # io.imsave(haze_img_path, haze_img)
                    # io.imsave(clear_img_path, clear_img)
