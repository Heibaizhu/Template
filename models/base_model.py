...
class BaseModel:
    ...
    def __init__(self, opt):
        self.opt = opt
        self.amp = self.opt.get('amp', False)
        self.opt['network_g']['amp'] = self.amp
    ...
    def setup_schedulers(self):
        ...
        elif hasattr(torch_lr_scheduler, scheduler_type):
            for optimizer in self.optimizers:
                self.schedulers.append(
                    getattr(torch_lr_scheduler, scheduler_type)(optimizer, **train_opt['scheduler'])
                )
        else:
    ...
    def update_learning_rate(self, current_iter, warmup_iter=-1, loss_total=0):
        ...
        for net_, param_key_ in zip(net, param_key):
            ...
            for key, param in state_dict.items():
                if key.startswith('module.'):  # remove unnecessary 'module.'
                    key = key[7:]
                if param is isinstance(param, torch.Tensor):
